...
import { Search, Send, Paperclip, Smile, MoreVertical, Phone, Video, ArrowLeft, Loader } from 'lucide-react';
...
const Messages = () => {
  ...
  const [isUserMsg, setIsUserMsg] = useState([]);
  const [isLoadingContacts, setIsLoadingContacts] = useState(true);
  const [searchParams] = useSearchParams();
  const location = useLocation();
  const chatUserFromState = location.state?.chatUser;
  const requestedChatUserId = Number(searchParams.get('user') || chatUserFromState?.id || 0);
    ...
    const listeners = [
      {
        ...
        handler: (e) => {
          ...
          setIsUserMsg(prev => {
            ...
            return flatPrev.includes(e.sender_id) ? flatPrev : [...flatPrev, e.sender_id];
          });
  ...
  const getUser = () => {
    ...
    axios.request(config)
      .then((response) => {
        let nextContacts = Array.isArray(response.data) ? response.data : [];

        if (requestedChatUserId && chatUserFromState && !nextContacts.some(contact => Number(contact.id) === requestedChatUserId)) {
          nextContacts = [chatUserFromState, ...nextContacts];
        }
  ...
  useEffect(() => {
    if (!requestedChatUserId || isLoadingContacts || contacts.length === 0) return;
    if (Number(receiverId) === requestedChatUserId && checkUser) return;

    const contactIndex = contacts.findIndex(contact => Number(contact.id) === requestedChatUserId);

    if (contactIndex !== -1) {
      openChat(contacts[contactIndex], contactIndex);
    }
  }, [requestedChatUserId, contacts, isLoadingContacts, receiverId, checkUser]);
            ...
            <div className={`bg-white ${checkUser ? 'hidden md:block md:w-80 md:border-r' : 'block w-full md:w-80 md:border-r'}`}>
              ...
              <div className="overflow-y-auto h-[calc(100%-57px)]">
                {isLoadingContacts ? (
                  ...
                  <div
                    key={getUser.id}
                    className={`p-3 border-b flex items-start cursor-pointer hover:bg-gray-50 transition-colors ${Number(receiverId) === Number(getUser.id) ? 'bg-gray-50' : ''}`}
                    onClick={() => {
                      openChat(getUser, index);
                      read();
                    }}
                  >
                    <div className="relative mr-3">
                      <Avatar onClick={changeUser} className="h-12 w-12">
                    ...
                    <div onClick={() => {
                      ...
                      <div className="flex justify-between items-baseline">
                        ...
                        <span className="text-xs text-gray-500">
                          {isUserMsg.includes(getUser.id) && (<span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full">xgdfgdf</span>)}
                        </span>
                      </div>
              ...
              {checkUser ? (
                <div className="flex h-full flex-col bg-[#f8f4fb]">
                  ...
                  <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white/95 px-3 py-3 backdrop-blur-sm md:static md:px-3 md:py-3">
                    <div className="flex items-center">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-500">{contacts[selectedChat]?.online ? 'Çevrimiçi' : 'Cevrimdisi'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    className="flex-1 overflow-y-auto px-3 py-4 space-y-3 bg-[#f8f4fb] md:p-4 md:space-y-4"
                  >
                          ...
                          {message.image && (
                            <>
                              <img
                                ...
                                width={100}
                                onClick={() => {
                                  setPreviewImageUrl(`${imageUrl}/${message.image}`);
                  ...
                  {previewImage && (
                    <div className="border-t border-border/60 bg-white px-3 pt-3">
                      <img src={previewImage} alt="Önizleme" className="max-w-[140px] rounded-2xl border border-border/60" style={{ width: "100px" }} />
