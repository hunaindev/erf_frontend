...
const Campaigns = () => {
  ...
  const handleGetSubscription = async () => {
    ...
    try {
      ...
      if (response.data.paytr && response.data.paytr?.status === "active") {
        setButtonState(true);
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
          ...
          >
            <div className="sticky top-0 z-20 -mx-4 mb-6 border-b border-slate-100 bg-white/95 px-4 pb-4 pt-2 backdrop-blur-sm sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:px-0 sm:pb-0 sm:pt-0">
              ...
              <TabsList className="grid h-auto w-full grid-cols-3 border-b border-gray-200 bg-transparent p-0">
                ...
                >
                  Taslak Kampanyalar{" "}
                  <span className="bg-orange-500 text-white rounded-full text-xs ml-1 px-1.5">
                    {campaigns?.campaign_draft
                      ? campaigns?.campaign_draft.length
                      : "0"}
                  </span>
                </TabsTrigger>
                <TabsTrigger
                ...
                >
                  Aktif Kampanyalar{" "}
                  <span className="bg-orange-500 text-white rounded-full text-xs ml-1 px-1.5">
                    {campaigns?.campaign_published
                      ? campaigns?.campaign_published.length
                      : "0"}
                  </span>
                </TabsTrigger>
                <TabsTrigger
                ...
                >
                  Geçmis Kampanyalar
                  <span className="bg-orange-500 text-white rounded-full text-xs ml-1 px-1.5">
                    {campaigns?.campaign_closed
                      ? campaigns?.campaign_closed.length
                      : "0"}
                  </span>
                </TabsTrigger>
              </TabsList>
            ...
            <TabsContent value="active" className="mt-0 pb-8 sm:pb-6 md:pb-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {isLoading ? (
                  ...
                  campaigns?.campaign_published.length > 0 ? (
            ...
            <TabsContent value="past" className="mt-0 pb-8 sm:pb-6 md:pb-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {isLoading ? (
                  ...
                  campaigns?.campaign_closed.length > 0 ? (
            ...
            <TabsContent value="draft" className="mt-0 pb-8 sm:pb-6 md:pb-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {isLoading ? (
                  ...
                  campaigns?.campaign_draft.length > 0 ? (
                  campaigns?.campaign_draft.map((campaign, key) => (
                    ...
                    >
                      <div
                        ...
                        onClick={() => handleDraftClick(campaign)}
                      >
                        <img
                          src={
                            campaign.image
                              ? `${imageUrl}/${campaign.image}`
                              : "https://res.cloudinary.com/dqtjuqw51/image/upload/assets_task_01jxzbw9eyeprv0r9hsdtf2qb3_1750178151_img_0_wywdib.webp"
                          }
                          alt={campaign.name}
                          className="w-full h-full object-cover"
                        />
      ...
      <CampaignForm onClose={handleCampaignFormClose} open={campaignFormOpen} />
