...
const DraftCampaignForm: React.FC<CampaignFormProps> = ({
  onClose,
  open,
  data,
}) => {
  ...
  const handleSubmit = (e) => {
    ...
    const config = {
      method: "post",
      maxBodyLength: Infinity,
      url: `${baseUrl}/api/create-draft-campaign`,
      headers: {
        Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}`,
      },
      data: formData,
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl">TASLAK KAMPANYA </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-4 mt-6">
            ...
            <div className="border border-border rounded p-2">
              ...
              <Textarea
                required
                id="requirements"
                className="min-h-32 border-0 focus-visible:ring-0 focus-visible:ring-offset-0"
                value={requirements}
          ...
          <div className="space-y-4 mt-6">
            ...
            >
              {isSubmitting ? "Kampanya Oluşturuluyor..." : "Kampanya Oluştur"}
            </Button>

            <p className="text-xs text-center text-muted-foreground">
              * Kampanya başlatarak şartlarımızı kabul etmiş olursunuz.{" "}
              <span className="text-primary cursor-pointer hover:underline ml-1">
                İçerik Kuralları ve Şartlar
              </span>
            </p>
          </div>
        </form>
